...
def neuralBalance(inl, oul, order):
    ...
    sizeRatio = inl.weight.shape[1]/oul.weight.shape[0]
    inl.weight.data *= optimal_l.unsqueeze(1)
    inl.weight.data /= sizeRatio
    oul.weight.data /= optimal_l
    oul.weight.data *= sizeRatio
...
def main():
    ...
    with open(f'/baldig/proteomics2/ian/Neural-Balance/personalExps/MnistFC/hist/MNISTLateral-model_{args.model}-lr_{args.lr}-l2Weight_{args.l2_weight}-seed_{args.seed}-neuralBalance_{args.neural_balance}-neuralBalanceAtStart_{args.neuralFullBalanceAtStart}-trainDataFrac_{args.trainDataFrac}.pkl', 'wb') as f:
        pickle.dump(hist, f)

    torch.save(model.state_dict(), f'/baldig/proteomics2/ian/Neural-Balance/personalExps/MnistFC/models/MNISTLateral-model_{args.model}-lr_{args.lr}-l2Weight_{args.l2_weight}-seed_{args.seed}-neuralBalance_{args.neural_balance}-neuralBalanceAtStart_{args.neuralFullBalanceAtStart}-trainDataFrac_{args.trainDataFrac}.pt')
