...
for model in ['small_fcn', 'medium_fcn', 'large_fcn']:
    for fbAtStart in ['0']:
        for iteration in range(5):
            ...
            command.append('2')
            command.append('--filename')
            command.append(f'{model}-L2NB-{iteration}-fbAtStart-{fbAtStart}-seed-{seed}-frac-{fract}')
            command.append(f'--foldername')
            command.append(f'{model}')
            command.append('--trainDataFrac')
            command.append(str(fract))
            ...
            command.append('1')
            command.append('--filename')
            command.append(f'{model}-L1NB-{iteration}-fbAtStart-{fbAtStart}-seed-{seed}-frac-{fract}')
            command.append(f'--foldername')
            command.append(f'{model}')
            command.append('--trainDataFrac')
            command.append(str(fract))
